...
class Arpcp_agent_server:
    ...
    def serve_forever(self):
        ...
        try:
            ...
            while True:
                ...
                socketfile = Arpcp.make_socket_file(conn, mode = "rwb", buffering = 0)
                try:
    ...
    def serve_client(self, conn, socketfile):
        try:
            arpcp_dict_message = Arpcp.parse_request_to_arpcp_format(socketfile) 
            print(arpcp_dict_message)
            # if 'Async' in headers.keys() and headers['Async']:
            #     self.create_async_task(starting_line, headers)
            # else:
                ...
                # successful_execution, result_of_task = self.perform_sync_task(starting_line, headers)
    ...
    # def perform_sync_task(self, starting_line, headers):
